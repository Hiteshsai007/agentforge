...
def record_execution(
    ...
    ca_result = (
        ...
        .select("quality_score, execution_count, settings")
    ...
    try:
        ...
        db.table("company_agents").update(update_data).eq("company_id", company_id).eq(
            "agent_id", agent_id
        ).execute()
    except Exception as e:
    ...
    if quality_privacy == "public":
        try:
            ...
            if agent_result.data:
                prev_global = float(
                    agent_result.data[0].get("global_quality_score") or 0.5
                )
                new_global = ALPHA * exec_score + (1 - ALPHA) * prev_global
                new_global = round(max(0.0, min(1.0, new_global)), 3)

                db.table("agents_marketplace").update(
                    {"global_quality_score": new_global}
...
def apply_specific_update(
    company_id: str, old_agent_id: str, new_agent_id: str
) -> bool:
    """
