...
async def signup(body: SignupRequest):
    ...
    if body.signup_type == "create_company":
        ...
        company_result = db.table("companies").select("*").eq("invite_code", body.invite_code.upper()).execute()
...
async def login(body: LoginRequest):
    ...
    if not user.get("password_hash"):
        raise HTTPException(status_code=401, detail="Account not set up properly. Please sign up again.")
    ...
    # Update last login
    db.table("users").update({"last_login": datetime.utcnow().isoformat()}).eq("user_id", user["user_id"]).execute()
    
    # Get company info if user has company
    company_data = {}
    if user.get("company_id"):
        company_result = db.table("companies").select("company_name, invite_code").eq("company_id", user["company_id"]).execute()
...
async def get_invite_code(authorization: str = Header(None)):
    ...
    user_result = db.table("users").select("company_id").eq("user_id", payload["user_id"]).execute()
    ...
    company_result = db.table("companies").select("company_name, invite_code").eq("company_id", company_id).execute()
    ...
    if not company.get("invite_code"):
        ...
        db.table("companies").update({"invite_code": new_code}).eq("company_id", company_id).execute()
...
async def get_me(authorization: str = None):
    ...
    # Get user
    user_result = db.table("users").select("*").eq("user_id", payload["user_id"]).execute()
    ...
    company_name = None
    if user.get("company_id"):
        company_result = db.table("companies").select("company_name").eq("company_id", user["company_id"]).execute()
        if company_result.data:
            company_name = company_result.data[0]["company_name"]
