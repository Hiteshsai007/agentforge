...
from ..models import AddAgentRequest, UpdateAgentSettingsRequest, CredentialGenerateRequest, CredentialResponse, CredentialStatusResponse, CredentialRotateRequest
...
async def update_agent_settings(company_id: str, agent_id: str, body: UpdateAgentSettingsRequest):
    ...
    try:
        current_result = db.table("company_agents").select("settings").eq(
            "company_id", company_id
...
@router.post("/{company_id}/agents/{agent_id}/generate-credentials", tags=["Credentials"])
async def generate_credentials(company_id: str, agent_id: str, body: Optional[CredentialGenerateRequest] = Body(default=None)):
    ...
    try:
        ...
        cred_result = db.table("agent_credentials").upsert({
            "company_id": company_id,
            "agent_id": agent_id,
        ...
        credential_id = cred_result.data[0]["credential_id"] if cred_result.data else None
        
        # Get company admin email
        users_result = db.table("users").select("email").eq(
            "company_id", company_id
...
async def get_credentials_status(company_id: str, agent_id: str):
    ...
    try:
        result = db.table("agent_credentials").select("*").eq(
            "company_id", company_id
        ).eq("agent_id", agent_id).execute()
        ...
        masked_key = CredentialManager.mask_api_key("sk_live_" + (cred.get("credential_id", "")[-4:] or "******"))
...
async def rotate_credentials(company_id: str, agent_id: str, body: Optional[CredentialRotateRequest] = Body(default=None)):
    ...
    try:
        ...
        return {
            "success": True,
            "credential": {
                "credential_id": cred_result.data[0]["credential_id"] if cred_result.data else "",
                "api_key": api_key,
                "secret_key": secret_key,
                "api_key_masked": CredentialManager.mask_api_key(api_key),
                "expiry_date": expiry_date.strftime("%Y-%m-%d"),
